...
Now, if we look at the contents of our current directory we have a folder called `data`, and a folder called `hannahs_unix_cafe`,
...
##### Practice:
```
Find the date runtime.txt was created.
```
...
Navigate into the `hannahs_unix_cafe` directory.
...
Here we see several directories, and one text file. We can see what is in the text file using the `cat` command which concatenates and prints the content of the file we list. 
...
1. use that `cat` function! check out the contents of `fall_desserts.txt`, and 2 more menus
...
Here, Im throwing way too much at you:
```
mkdir ~/data
cd ~/data
```
...
makes compressed representation of genomes (magic of kmers)
```
...
lets try this with more reference ecoli genomes
```
...
```
The point the lesson today is not for you to be able to make sense of this output. The point is four you to understand that there are softwares that you might want to use, that require the use of a CLI (command line interface). May of these tools can output a table that we can use in R!
...
check the box of the `ecoli.cmp.csv`, 
